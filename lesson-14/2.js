...
function CleanerRobot(initialEnergy = 0 /* Изначальный заряд батареи */) {
  ...
  function setEnergy(initialEnergy) {
    if(initialEnergy < 0 || initialEnergy > MAX_ENERGY_CAPACITY){
      throw new Error(`1New energy level can not be less then 0 and bigger then ${MAX_ENERGY_CAPACITY}`)
    }
    this.energy = initialEnergy;
    console.log(`Текущий заряд батареи: ${this.getEnergy()}`)
  }
